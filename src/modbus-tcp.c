...
static ssize_t _modbus_tcp_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length) 
{
	...
	while ( rc<=0 && !caribou_timer_ticks_timeout(start,from_ms(2000)) )	/* FIXME - get timeout from settings */
	{
		rc = lwip_recv(ctx->s, (char *)rsp, rsp_length, MSG_DONTWAIT);
...
int modbus_tcp_listen(modbus_t *ctx, int nb_connection)
{
    ...
    new_s = lwip_socket(AF_INET, SOCK_STREAM, 0);
    if (new_s == -1) {
        return -1;
    }
