...
static int _modbus_tcp_init_win32(void)
{
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        MODBUS_FPRINTF(stderr, "WSAStartup() returned error code %d\n",
...
static int _modbus_tcp_pi_connect(modbus_t *ctx)
{
    ...
    if (rc != 0) {
        if (ctx->debug) {
            MODBUS_FPRINTF(stderr, "Error returned by getaddrinfo: %s\n", gai_strerror(rc));
        }
        errno = ECONNREFUSED;
        return -1;
    }
...
modbus_t* modbus_new_tcp(const char *ip, int port)
{
    ...
    if (sigaction(SIGPIPE, &sa, NULL) < 0) {
        ...
        MODBUS_FPRINTF(stderr, "Coud not install SIGPIPE handler.\n");
        return NULL;
    }
#endif
...
modbus_t* modbus_new_tcp_pi(const char *node, const char *service)
{
    ...
    if (ret_size == 0) {
        MODBUS_FPRINTF(stderr, "The node string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size >= dest_size) {
        MODBUS_FPRINTF(stderr, "The node string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size == 0) {
        MODBUS_FPRINTF(stderr, "The service string is empty\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
    ...
    if (ret_size >= dest_size) {
        MODBUS_FPRINTF(stderr, "The service string has been truncated\n");
        modbus_free(ctx);
        errno = EINVAL;
        return NULL;
    }
