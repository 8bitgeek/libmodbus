...
/* Exported version */
extern "C" const unsigned int libmodbus_version_major = LIBMODBUS_VERSION_MAJOR;
extern "C" const unsigned int libmodbus_version_minor = LIBMODBUS_VERSION_MINOR;
extern "C" const unsigned int libmodbus_version_micro = LIBMODBUS_VERSION_MICRO;
...
extern "C" const char *modbus_strerror(int errnum) {
    switch (errnum) {
    case EMBXILFUN:
...
extern "C" void _error_print(modbus_t *ctx, const char *context)
{
    if (ctx->debug) {
...
extern "C" int modbus_flush(modbus_t *ctx)
{
    int rc;
...
extern "C" int modbus_send_raw_request(modbus_t *ctx, uint8_t *raw_req, int raw_req_length)
{
    sft_t sft;
                                          ...
                                          msg_type_t msg_type)
{
	...
	*/
	extern "C" int _modbus_rtu_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
	{
		int rc;
		int length_to_read;
		int msg_length = 0;
		...
		{
			if ( msg_type == MSG_INDICATION )
				gSyslog->log(DEBUG_MODBUSDEVICE,"Waiting for a indication...");
			else
				gSyslog->log(DEBUG_MODBUSDEVICE,"Waiting for a confirmation...");
		}
		...
		caribou_tick_t response_start = caribou_timer_ticks();
		caribou_tick_t start_last_char = 0;

		while (length_to_read != 0 && !caribou_timer_ticks_timeout(response_start,ctx->response_timeout) ) 
		{
			...
			if (rc > 0) 
			{
					...
					switch (step) 
					{
						...
						case _STEP_META:
							...
							if ((msg_length + length_to_read) > (int)ctx->backend->max_adu_length) 
							{
								...
								if (ctx->debug)
								{
									gSyslog->log(DEBUG_MODBUSDEVICE,"** %d is too much data", msg_length + length_to_read);
								}
								return -1;
							}
							step = _STEP_DATA;
				...
				start_last_char = caribou_timer_ticks();
				response_start = caribou_timer_ticks();
			}
			else if ( start_last_char != 0L )
			{
				if ( caribou_timer_ticks_timeout(start_last_char,ctx->byte_timeout) )
				{
					if (ctx->debug)
					{
						/* Display the hex code of each character received */
						gSyslog->log(DEBUG_MODBUSDEVICE,"** byte timeout: msg_length=%d length_to_read=%d",msg_length,length_to_read);
					}
					errno = ETIMEDOUT;
					return -1;
				}
		...
		return ( msg_length >= ctx->backend->header_length + 1 ) ? ctx->backend->check_integrity(ctx, msg, msg_length) : -1;
	}
#endif
...
extern "C" int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
	#if defined(CARIBOU_RTOS)
		...
		else
		{
			...
			while (length_to_read != 0 && !caribou_timer_ticks_timeout(start,from_ms(ctx->response_timeout)) ) 
			{
				rc = ctx->backend->recv(ctx, msg + msg_length, length_to_read);
				if (rc == -1) {
				...
				if (ctx->debug) {
					int i;
					for (i=0; i < rc; i++)
						debug_printf("<%02X>", msg[msg_length + i]);
			...
			if (length_to_read == 0) {
				...
				if (length_to_read > 0 && ctx->byte_timeout > 0) 
				{
					/* If there is no character in the buffer, the allowed timeout
					   ...
					   byte_timeout */
					tv.tv_sec = 0;
					tv.tv_usec = ctx->byte_timeout * 1000;
					p_tv = &tv;
				}
...
extern "C" int modbus_receive(modbus_t *ctx, uint8_t *req)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
/* Receives the confirmation.
   ...
   initial request.
*/
extern "C" int modbus_receive_confirmation(modbus_t *ctx, uint8_t *rsp)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
/* Send a response to the received request.
   ...
   accordingly.
*/
extern "C" int modbus_reply(modbus_t *ctx, const uint8_t *req, int req_length, modbus_mapping_t *mb_mapping)
{
...
extern "C" int modbus_reply_exception(modbus_t *ctx, const uint8_t *req, unsigned int exception_code)
{
    int offset = ctx->backend->header_length;
    int slave = req[offset - 1];
    int function = req[offset];
...
extern "C" int modbus_read_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest)
{
    int rc;
...
/* Reads the holding registers of remote device and put the data into an
   array */
extern "C" int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest)
{
    int status;
...
extern "C" int modbus_read_input_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest)
{
    int status;
...
extern "C" int modbus_write_bit(modbus_t *ctx, int addr, int status)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" int modbus_write_register(modbus_t *ctx, int addr, int value)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    int rc;
    int i;
    int byte_count;
...
extern "C" int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src)
{
	int rc;
	int i;
	int req_length;
	...
	rc = send_msg(ctx, req, req_length);
    if ( req[0]!='\0') /* don't wait for reply from */
	{
		if (rc > 0) {
			uint8_t rsp[MAX_MESSAGE_LENGTH];
...
extern "C" int modbus_mask_write_register(modbus_t *ctx, int addr, uint16_t and_mask, uint16_t or_mask)
{
    int rc;
    int req_length;
    uint8_t req[_MIN_REQ_LENGTH];
...
extern "C" int modbus_write_and_read_registers(modbus_t *ctx,
...
/* Send a request to get the slave ID of the device (only available in serial
   communication). */
extern "C" int modbus_report_slave_id(modbus_t *ctx, int max_dest, uint8_t *dest)
{
    int rc;
    int req_length;
    uint8_t req[_MIN_REQ_LENGTH];
...
extern "C" void _modbus_init_common(modbus_t *ctx)
{
...
extern "C" int modbus_set_error_recovery(modbus_t *ctx, modbus_error_recovery_mode error_recovery)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" int modbus_set_socket(modbus_t *ctx, int s)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" int modbus_get_socket(modbus_t *ctx)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" int modbus_get_response_timeout(modbus_t *ctx, uint32_t *to_sec, uint32_t *to_usec)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" int modbus_set_response_timeout(modbus_t *ctx, uint32_t to_sec, uint32_t to_usec)
{
    if (ctx == NULL ||
...
extern "C" int modbus_get_byte_timeout(modbus_t *ctx, uint32_t *to_sec, uint32_t *to_usec)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" int modbus_set_byte_timeout(modbus_t *ctx, uint32_t to_sec, uint32_t to_usec)
{
...
extern "C" int modbus_get_header_length(modbus_t *ctx)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" void modbus_close(modbus_t *ctx)
{
    if (ctx == NULL)
        return;
...
extern "C" void modbus_free(modbus_t *ctx)
{
    if (ctx == NULL)
        return;
...
extern "C" int modbus_set_debug(modbus_t *ctx, int flag)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
...
extern "C" modbus_mapping_t* modbus_mapping_new(int nb_bits, int nb_input_bits, int nb_registers, int nb_input_registers)
{
...
/* Frees the 4 arrays */
extern "C" void modbus_mapping_free(modbus_mapping_t *mb_mapping)
{
    if (mb_mapping == NULL) {
        return;
    }
 ...
 */
extern "C" size_t strlcpy(char *dest, const char *src, size_t dest_size)
{
